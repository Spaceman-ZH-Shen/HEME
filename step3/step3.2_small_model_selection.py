...
OUTPUT_JSON_PATH = path_args.project_path + "/knowledge/step3_output_1120_small_model.json"
...
def draw_bounding_boxes_with_labels(image_path: str, bboxes: Dict[str, List]) -> str:
    ...
    colors = {
        'florence_ft_bbox': 'green',
        'florence_bbox': 'darkgreen',
        'gdino_bbox': 'purple',
        'llmdet_bbox': 'cyan'
    }
    model_names = {
        'florence_ft_bbox': 'Florence-FT',
        'florence_bbox': 'Florence',
        'gdino_bbox': 'G-DINO',
        'llmdet_bbox': 'LLMDet'
    }
    try:
...
def build_bbox_selection_messages(image_path: str, question: str, available_bboxes: Dict[str, List], 
    ...
    for bbox_key, bbox in available_bboxes.items():
        if bbox:
            ...
            bbox_descriptions.append(f"- {name} (marked in {color} color)")
...
**Your task:**
1. **First**, carefully evaluate whether the bounding box from **Step2-1120** accurately and precisely localizes the target object described in the question.
...
- Avoid selecting placeholder or empty boxes, if you do not think there is a correct answer, please select 'NONE'.

**Output format (choose only one):**
SELECTED: [Model Name] 
(e.g., SELECTED: G-DINO, or if you think all answers are wrong, output 'SELECTED: NONE')
    ...
    try:
        for line in response.splitlines():
            if 'SELECTED:' in line.upper():
                ...
                if 'NONE' in selected:
                    return 'NONE'           
                if 'FLORENCE-FT' in selected: return 'florence_ft_bbox'
                if 'FLORENCE'    in selected: return 'florence_bbox'
                if 'G-DINO' in selected or 'GDINO' in selected: return 'gdino_bbox'
                if 'LLMDET'     in selected: return 'llmdet_bbox'
                return None
        return None
        ...
        for entry in tqdm(bbox_data, desc="选择最佳边界框", ncols=100):
            ...
            try:
                ...
                qwen_evaluation, selected_model_key = select_best_bbox(
                    image_with_boxes_path, enhanced_question, available_bboxes,
                    setting, main_prompt, spatial_desc
                )
                if selected_model_key in (None, 'NONE'):
                    print(f"Skipping {image_name} – Qwen did not select a valid bbox")
                    continue
                ...
                result_entry = {
                    ...
                    "has_additional_context": bool(setting or main_prompt or spatial_desc)
                }
...
#############################################################
    input_file = path_args.project_path + "/knowledge/step3_output_1120_small_model.json"
    image_folder = 'images'
    output_json_path = path_args.project_path + "/knowledge/step3_output_1120_small_model.json"  # 你想保存的新文件路径
